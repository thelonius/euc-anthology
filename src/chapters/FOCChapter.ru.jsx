  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '14px', fontSize: '11px', color: '#444', lineHeight: 1.6 }}>
        {wrapTerms('Жёлтым пунктиром — целевой ток Iq (square wave 0 ↔ 200 А). Зелёный — измеренный Iq, оранжевый — измеренный Id (цель: 0 А). При выключенном БКПС: каждое скачкообразное изменение Iq возмущает Id через перекрёстный терм ')}
        <em>ωe·Lq·Iq</em>
        {wrapTerms(' в уравнении d-оси. При включённом — feed-forward компенсация добавляет к выходу PI заранее вычисленные «помеховые» термы, и Id остаётся чистым. Эффект сильнее при больших ωe — попробуйте сравнить на 200 рад/с и 2000 рад/с.')}
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '14px', fontSize: '11px', color: '#444', lineHeight: 1.7 }}>
        {wrapTerms('Сверху — три фазных напряжения относительно средней точки шины. Снизу — междуфазное V_AB, которое реально приложено к обмотке. В SVPWM из каждой фазы вычитается общая (синфазная, common-mode) составляющая (max+min)/2, что «сжимает» три синусоиды в полосу ±V_bus/2, одновременно увеличивая допустимый modulation index с 1.00 до 2/√3 ≈ 1.155 — это +15.5% к мощности на той же шине. Поскольку синфазное вычитание одинаково во всех фазах, междуфазное (нижний график) не меняется — мотор видит то же самое. Поднимите m до 1.1 в режиме «Синусная» — увидите как фазы упираются в рельсы и форма искажается (overmodulation).')}
      </div>
    </div>
  )
}
